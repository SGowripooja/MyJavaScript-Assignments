  ...
  if (req.body.startTime) {
    if (time_regex.test(req.body.startTime)) {
      ...
      res
        ...
        .json({
          output:
            "Please specify the start time in hh:min(24-hr format) where minutes should be 00 only",
        });
    }
  } else {
    res
  ...
  if (req.body.endTime) {
    if (time_regex.test(req.body.endTime)) {
      ...
      res
        ...
        .json({
          output:
            "Please specify the End time in hh:min(24-hr format) where minutes should be 00 only",
        });
    }
  } else {
